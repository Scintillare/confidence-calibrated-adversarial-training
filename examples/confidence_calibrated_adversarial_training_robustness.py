...
class Main:
    ...
    def get_test_attacks(self):
        ...
        ours.backtrack = True
        ours.initialization = attacks.initializations.LInfUniformNormInitialization(self.epsilon)
    ...
    def train(self):
        ...
        transition = common.utils.partial(common.torch.power_transition, epsilon=self.epsilon, norm=attacks.norms.LInfNorm(), gamma=10)
        loss = common.torch.cross_entropy_divergence
        trainer = common.train.ConfidenceCalibratedAdversarialTraining(self.model, self.trainloader, self.testloader, optimizer, scheduler, attack, objective,
                                                                       loss, transition, augmentation=augmentation, writer=writer, cuda=self.args.cuda)
    ...
    def evaluate(self):
        ...
        for attack, objective, attempts in self.get_test_attacks():
            _, adversarial_probabilities, _ = common.test.attack(self.model, self.adversarialloader,
                                                                 attack, objective, attempts=attempts, cuda=self.args.cuda)
